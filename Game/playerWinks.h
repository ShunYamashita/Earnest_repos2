...
//--------------------------------------------------------------------------------------
#ifndef _PLAYER_WINKS_H_
#define _PLAYER_WINKS_H_
...
//  プレイヤークラスの定義
//--------------------------------------------------------------------------------------
class PlayerWinks : public Scene
{
public:
	enum class STATE														//  状態
	{
	...
	static PlayerWinks*		Create( D3DXVECTOR3 position ,					//  生成
									D3DXVECTOR3 rot ,
									D3DXVECTOR3 scale ,
									float fSpeed ,
									MAGIC magic , 
									int nPlayerWinksNo = 0 );
	...
	void				ChangeState( PlayerWinksState* playerState );		//  状態の変化
	...
	PlayerWinksState*	GetPlayerState( STATE state );						//  状態の取得
	...
	FBXLoader*			GetFBXLoader( void ){ return m_fbx; }				//  FBXの取得	
...
private:
	...
	PlayerUI*			m_pPlayerWinksUI;									//  プレイヤーUIクラスのポインタ
	Magic*				m_pMagic;											//  魔法クラスのポインタ
	FBXLoader*			m_fbx;												//  FBXデータのポインタ

	PlayerWinksState*	m_playerState;										//  プレイヤー状態クラスのポインタ

	PlayerWinksState*	m_allState[ ( int )STATE::MAX ];					//  状態全ての保存用
