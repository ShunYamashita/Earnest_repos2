...
void ImGui_ImplDX9_RenderDrawLists(ImDrawData* draw_data)
{
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        {
            vtx_dst->position[0] = vtx_src->position.x;
            vtx_dst->position[1] = vtx_src->position.y;
...
{
    ImGui_ImplDX9_InvalidateDeviceScenes();
    ImGui::Shutdown();
    g_pd3dDevice = NULL;
    g_hWnd = 0;
}
...
bool ImGui_ImplDX9_CreateDeviceScenes()
{
    if (!g_pd3dDevice)
        return false;
...
void ImGui_ImplDX9_InvalidateDeviceScenes()
{
    if (!g_pd3dDevice)
        return;
    if (g_pVB)
...
{
    if (!g_FontTexture)
        ImGui_ImplDX9_CreateDeviceScenes();
