...
/*
 ...
 - A minimal render function skeleton may be:
    ...
    void void MyRenderFunction(ImDrawData* draw_data)(ImDrawData* draw_data)
    {
       ...
       // TODO: Setup shader: vertex { float2 position, float2 uv, u32 color }, fragment shader sample color from 1 texture, multiply by vertex color.
 ...
 - 2014/12/10 (1.18) - removed SetNewWindowDefaultPos() in favor of new generic API SetNextWindowPos(position, ImGuiSetCondition_FirstUseEver)
...
static void             SetWindowPos(ImGuiWindow* window, const ImVec2& position, ImGuiCond cond);
static void             SetWindowSize(ImGuiWindow* window, const ImVec2& size, ImGuiCond cond);
static void             SetWindowCollapsed(ImGuiWindow* window, bool collapsed, ImGuiCond cond);
static ImGuiWindow*     FindHoveredWindow(ImVec2 position, bool excluding_childs);
...
void ImGui::Render()
{
    ...
    if (g.Style.Alpha > 0.0f)
    {
        ...
        if (g.IO.MouseDrawCursor)
        {
            ...
            const ImVec2 position = g.IO.MousePos - cursor_data.HotOffset;
            const ImVec2 size = cursor_data.Size;
            const ImTextureID tex_id = g.IO.Fonts->TexID;
            g.OverlayDrawList.PushTextureID(tex_id);
            g.OverlayDrawList.AddImage(tex_id, position+ImVec2(1,0), position+ImVec2(1,0) + size, cursor_data.TexUvMin[1], cursor_data.TexUvMax[1], IM_COL32(0,0,0,48));        // Shadow
            g.OverlayDrawList.AddImage(tex_id, position+ImVec2(2,0), position+ImVec2(2,0) + size, cursor_data.TexUvMin[1], cursor_data.TexUvMax[1], IM_COL32(0,0,0,48));        // Shadow
            g.OverlayDrawList.AddImage(tex_id, position,             position + size,             cursor_data.TexUvMin[1], cursor_data.TexUvMax[1], IM_COL32(0,0,0,255));       // Black border
            g.OverlayDrawList.AddImage(tex_id, position,             position + size,             cursor_data.TexUvMin[0], cursor_data.TexUvMax[0], IM_COL32(255,255,255,255)); // White fill
...
void ImGui::RenderTextClipped(const ImVec2& pos_min, const ImVec2& pos_max, const char* text, const char* text_end, const ImVec2* text_size_if_known, const ImVec2& align, const ImRect* clip_rect)
{
    ...
    if (clip_rect) // If we had no explicit clipping rectangle then position==clip_min
    ...
    if (need_clipping)
    {
        ...
        window->DrawList->AddText(g.Font, g.FontSize, position, GetColorU32(ImGuiCol_Text), text, text_display_end, 0.0f, &fine_clip_rect);
    }
    else
    {
        window->DrawList->AddText(g.Font, g.FontSize, position, GetColorU32(ImGuiCol_Text), text, text_display_end, 0.0f, NULL);
    }
    if (g.LogEnabled)
        LogRenderedText(position, text, text_display_end);
}
...
void ImGui::RenderBullet(ImVec2 position)
{
    ImGuiWindow* window = GetCurrentWindow();
    window->DrawList->AddCircleFilled(position, GImGui->FontSize*0.20f, GetColorU32(ImGuiCol_Text), 8);
}

void ImGui::RenderCheckMark(ImVec2 position, ImU32 col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    float start_x = (float)(int)(g.FontSize * 0.307f + 0.5f);
    float rem_third = (float)(int)((g.FontSize - start_x) / 3.0f);
    float bx = position.x + 0.5f + start_x + rem_third;
    float by = position.y - 1.0f + (float)(int)(g.Font->Ascent * (g.FontSize / g.Font->FontSize) + 0.5f) + (float)(int)(g.Font->DisplayOffset.y);
...
void ImGui::CalcListClipping(int items_count, float items_height, int* out_items_display_start, int* out_items_display_end)
{
    ...
    int start = (int)((window->ClipRect.Min.y - position.y) / items_height);
    int end = (int)((window->ClipRect.Max.y - position.y) / items_height);
...
ImVec2 ImGui::CalcItemRectClosestPoint(const ImVec2& position, bool on_edge, float outward)
{
    ImGuiWindow* window = GetCurrentWindowRead();
    ImRect rect = window->DC.LastItemRect;
    rect.Expand(outward);
    return rect.GetClosestPoint(position, on_edge);
}
...
void ImGui::SetNextWindowPos(const ImVec2& position, ImGuiCond cond)
{
    ImGuiContext& g = *GImGui;
    g.SetNextWindowPosVal = position;
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& position, float radius)
{
    ImGuiWindow* window = GetCurrentWindow();

    const ImRect bb(position - ImVec2(radius,radius), position + ImVec2(radius,radius));
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    ImVec2 position = window->DC.CursorPos;
    ImRect bb(position, position + CalcItemSize(size_arg, CalcItemWidth(), g.FontSize + style.FramePadding.y*2.0f));
    ItemSize(bb, style.FramePadding.y);
    if (!ItemAdd(bb, NULL))
        return;
...
static void STB_TEXTEDIT_DELETECHARS(STB_TEXTEDIT_STRING* obj, int position, int n)
{
    ImWchar* dst = obj->Text.Data + position;
    ...
    const ImWchar* src = obj->Text.Data + position + n;
    while (ImWchar c = *src++)
        *dst++ = c;
    *dst = '\0';
}

static bool STB_TEXTEDIT_INSERTCHARS(STB_TEXTEDIT_STRING* obj, int position, const ImWchar* new_text, int new_text_len)
{
...
void ImGuiTextEditCallbackData::DeleteChars(int position, int bytes_count)
{
    ...
    *dst = '\0';
    ...
    if (CursorPos + bytes_count >= position)
...
void ImGuiTextEditCallbackData::InsertChars(int position, const char* new_text, const char* new_text_end)
{
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    ImVec2 position = window->DC.CursorPos;
    position.y += window->DC.CurrentLineTextBaseOffset;
    ...
    ImRect bb_with_spacing(position, position + size_draw);
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool selected, bool enabled)
{
    ...
    if (shortcut_size.x > 0.0f)
    {
        ...
        RenderText(position + ImVec2(window->MenuColumns.Pos[1] + extra_w, 0.0f), shortcut, NULL, false);
        PopStyleColor();
    }

    if (selected)
        RenderCheckMark(position + ImVec2(window->MenuColumns.Pos[2] + extra_w + g.FontSize * 0.20f, 0.0f), GetColorU32(enabled ? ImGuiCol_Text : ImGuiCol_TextDisabled));
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    {
        popup_pos = ImVec2(position.x - window->WindowPadding.x, position.y - style.FramePadding.y + window->MenuBarHeight());
    ...
    else
    {
        ...
        RenderCollapseTriangle(position + ImVec2(window->MenuColumns.Pos[2] + extra_w + g.FontSize * 0.20f, 0.0f), false);
...
// 'position' is position of the arrow tip. half_sz.x is length from base to tip. half_sz.y is length on each side.
static void RenderArrow(ImDrawList* draw_list, ImVec2 position, ImVec2 half_sz, ImGuiDir direction, ImU32 col)
{
    switch (direction)
    {
...
static void RenderArrowsForVerticalBar(ImDrawList* draw_list, ImVec2 position, ImVec2 half_sz, float bar_w)
{
    RenderArrow(draw_list, ImVec2(position.x + half_sz.x + 1,         position.y), ImVec2(half_sz.x + 2, half_sz.y + 1), ImGuiDir_Right, IM_COL32_BLACK);
    RenderArrow(draw_list, ImVec2(position.x + half_sz.x,             position.y), half_sz,                              ImGuiDir_Right, IM_COL32_WHITE);
    RenderArrow(draw_list, ImVec2(position.x + bar_w - half_sz.x - 1, position.y), ImVec2(half_sz.x + 2, half_sz.y + 1), ImGuiDir_Left,  IM_COL32_BLACK);
    RenderArrow(draw_list, ImVec2(position.x + bar_w - half_sz.x,     position.y), half_sz,                              ImGuiDir_Left,  IM_COL32_WHITE);
}
        ...
        struct Funcs
        {
            static void NodeDrawList(ImDrawList* draw_list, const char* label)
            {
                ...
                for (const ImDrawCmd* pcmd = draw_list->CmdBuffer.begin(); pcmd < draw_list->CmdBuffer.end(); elem_offset += pcmd->ElemCount, pcmd++)
                {
                    ...
                    bool pcmd_node_open = ImGui::TreeNode((void*)(pcmd - draw_list->CmdBuffer.begin()), "Draw %-4d %s vtx, texcoord = %p, clip_rect = (%.0f,%.0f)..(%.0f,%.0f)", pcmd->ElemCount, draw_list->IdxBuffer.Size > 0 ? "indexed" : "non-indexed", pcmd->TextureId, pcmd->ClipRect.x, pcmd->ClipRect.y, pcmd->ClipRect.z, pcmd->ClipRect.w);
                    if (show_clip_rects && ImGui::IsItemHovered())
                    {
                        ImRect clip_rect = pcmd->ClipRect;
                        ImRect vtxs_rect;
                        for (int i = elem_offset; i < elem_offset + (int)pcmd->ElemCount; i++)
                            vtxs_rect.Add(draw_list->VtxBuffer[idx_buffer ? idx_buffer[i] : i].position);
                    ...
                    while (clipper.Step())
                        for (int prim = clipper.DisplayStart, vtx_i = elem_offset + clipper.DisplayStart*3; prim < clipper.DisplayEnd; prim++)
                        {
                            ...
                            for (int n = 0; n < 3; n++, vtx_i++)
                            {
                                ...
                                triangles_pos[n] = v.position;
                                buf_p += sprintf(buf_p, "%s %04d { position = (%8.2f,%8.2f), uv = (%.6f,%.6f), col = %08X }\n", (n == 0) ? "vtx" : "   ", vtx_i, v.position.x, v.position.y, v.uv.x, v.uv.y, v.col);
                            }
