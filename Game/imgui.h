...
namespace ImGui
{
    ...
    IMGUI_API void          SetNextWindowPos(const ImVec2& position, ImGuiCond cond = 0);            // set next window position. call before Begin()
    ...
    IMGUI_API void          SetCursorScreenPos(const ImVec2& position);                              // cursor position in absolute screen coordinates [0..io.DisplaySize]
    ...
    IMGUI_API ImVec2        CalcItemRectClosestPoint(const ImVec2& position, bool on_edge = false, float outward = +0.0f);   // utility to find the closest point the last item bounding rectangle edge. useful to visually link items
...
// The code expect ImVec2 position (8 bytes), ImVec2 uv (8 bytes), ImU32 col (4 bytes), but you can re-order them or add other fields as needed to simplify integration in your engine.
...
struct ImDrawList
{
    ...
    inline    void  PathLineToMergeDuplicate(const ImVec2& position)                 { if (_Path.Size == 0 || memcmp(&_Path[_Path.Size-1], &position, 8) != 0) _Path.push_back(position); }
    ...
    inline    void  PrimVtx(const ImVec2& position, const ImVec2& uv, ImU32 col)     { PrimWriteIdx((ImDrawIdx)_VtxCurrentIdx); PrimWriteVtx(position, uv, col); }
...
struct ImFont
{
    ...
    IMGUI_API void              RenderChar(ImDrawList* draw_list, float size, ImVec2 position, ImU32 col, unsigned short c) const;
    IMGUI_API void              RenderText(ImDrawList* draw_list, float size, ImVec2 position, ImU32 col, const ImVec4& clip_rect, const char* text_begin, const char* text_end, float wrap_width = 0.0f, bool cpu_fine_clip = false) const;
