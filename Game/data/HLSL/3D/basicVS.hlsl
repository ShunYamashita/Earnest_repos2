...
float4		lightDiffuseColor;				//  ライトの拡散光
...
OUT_VERTEX main( IN_VERTEX input )
{
	...
	output.color.x = min( lightness * lightDiffuseColor.x + 0.3f , 1.0f );
	output.color.y = min( lightness * lightDiffuseColor.y + 0.3f , 1.0f );
	output.color.z = min( lightness * lightDiffuseColor.z + 0.3f , 1.0f );
	output.color.w = 1.0f;
