...
void ImGui::ShowTestWindow(bool* p_open)
{
    ...
    if (ImGui::CollapsingHeader("Widgets"))
    {
        ...
        if (ImGui::TreeNode("Text"))
        {
            ...
            if (ImGui::TreeNode("Word Wrapping"))
            {
                ...
                ImGui::GetWindowDrawList()->AddRectFilled(ImVec2(position.x + wrap_width, position.y), ImVec2(position.x + wrap_width + 10, position.y + ImGui::GetTextLineHeight()), IM_COL32(255,0,255,255));
    ...
    if (ImGui::CollapsingHeader("Layout"))
    {
        ...
        if (ImGui::TreeNode("Clipping"))
        {
            ...
            ImVec2 position = ImGui::GetCursorScreenPos();
            ImVec4 clip_rect(position.x, position.y, position.x+size.x, position.y+size.y);
            ImGui::InvisibleButton("##dummy", size);
            if (ImGui::IsItemActive() && ImGui::IsMouseDragging()) { offset.x += ImGui::GetIO().MouseDelta.x; offset.y += ImGui::GetIO().MouseDelta.y; }
            ImGui::GetWindowDrawList()->AddRectFilled(position, ImVec2(position.x+size.x,position.y+size.y), ImColor(90,90,120,255));
            ImGui::GetWindowDrawList()->AddText(ImGui::GetFont(), ImGui::GetFontSize()*2.0f, ImVec2(position.x+offset.x,position.y+offset.y), ImColor(255,255,255,255), "Line 1 hello\nLine 2 clip me!", NULL, 0.0f, &clip_rect);
            ImGui::TreePop();
        }
    }
...
static void ShowExampleAppPropertyEditor(bool* p_open)
{
    ...
    struct funcs
    {
        static void ShowDummyScene(const char* prefix, int uid)
        {
            ImGui::PushID(uid);                      // Use object uid as identifier. Most commonly you could also use the object pointer as a base ID.
            ImGui::AlignFirstTextHeightToWidgets();  // Text and Tree nodes are less high than regular widgets, here we add vertical spacing to make the tree lines equal high.
            bool node_open = ImGui::TreeNode("Scene", "%s_%u", prefix, uid);
            ImGui::NextColumn();
            ImGui::AlignFirstTextHeightToWidgets();
    ...
    for (int obj_i = 0; obj_i < 3; obj_i++)
        funcs::ShowDummyScene("Scene", obj_i);
