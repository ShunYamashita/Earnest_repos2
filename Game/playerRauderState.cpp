//--------------------------------------------------------------------------------------
//  プレイヤーステート  ( playerRauderState.cpp )
//
...
//  重力をかける関数
//--------------------------------------------------------------------------------------
void PlayerRauderState::Init( void )
{
	m_moveHeight = 0.0f;
	m_pressTime = 0;
}
...
//  プレイヤーデータの設定をする関数
//--------------------------------------------------------------------------------------
void PlayerRauderState::SetPlayer( PlayerRauder* playerRauder )
{
	if( playerRauder == nullptr )
	{
		return;
	}

	m_player = playerRauder;
}

//--------------------------------------------------------------------------------------
//  プレイヤーデータの取得をする関数
//--------------------------------------------------------------------------------------
PlayerRauder* PlayerRauderState::GetPlayer( void )
{
	if( m_player == nullptr )
	{
		return nullptr;
...
//  重力をかける関数
//--------------------------------------------------------------------------------------
void PlayerRauderState::AddGravity( void )
{
	//  地面との当たっていない場合
...
//  カメラ制御をする関数
//--------------------------------------------------------------------------------------
void PlayerRauderState::ControlCamera( void )
{
	//  カメラの取得
		...
		for( int nCntPriority = 0; nCntPriority < MAX_NUM_PRIORITY; nCntPriority++ )
		{
			...
			while( pScene != NULL )
			{
				...
				if( Game::GetModeVS( ) == Game::MODE_VS_PLAYER )
				{
					...
					{
						//  ダウンキャスト
						PlayerRauder* pPlayer = ( PlayerRauder* )pScene;
...
//  ロックオン状態の変更をする関数
//--------------------------------------------------------------------------------------
void PlayerRauderState::ChangeLockon( void )
{
	// キーボード情報の取得
	Keyboard* pKeyboard = SceneManager::GetKeyboard( );
...
//  待機状態に変更するかの判断をする関数
//--------------------------------------------------------------------------------------
void PlayerRauderState::JudgeChangeIdle( void )
{
	//  モーションが終了している場合
...
//  移動状態に変更するか判断する関数
//--------------------------------------------------------------------------------------
void PlayerRauderState::JudgeChangeMove( void )
{
	// キーボード情報の取得
	Keyboard* pKeyboard = SceneManager::GetKeyboard( );
	...
	{
		//  移動モーションに変更
		GetPlayer( )->SetMotion( FBXLoader::MOTION::RUN );

		//  ステートの変更
		GetPlayer( )->ChangeState( GetPlayer( )->GetPlayerState( PlayerRauder::STATE::MOVE ) );
	}
}
...
//  ジャンプ状態に変更するか判断する関数
//--------------------------------------------------------------------------------------
void PlayerRauderState::JudgeChangeJump( void )
{
	// キーボード情報の取得
	Keyboard* pKeyboard = SceneManager::GetKeyboard( );
	...
	if( pKeyboard->GetKeyboardTrigger( nKey ) )
	{
		...
		//  ジャンプ状態に
		GetPlayer( )->ChangeState( GetPlayer( )->GetPlayerState( PlayerRauder::STATE::JUMP ) );
	}
...
//  攻撃状態に変更するかの判断する関数
//--------------------------------------------------------------------------------------
void PlayerRauderState::JudgeChangeAttack( void )
{
	int nKey = 0;
	...
	{ 
		if( pKeyboard->GetKeyboardTrigger( nKey ) )
		{
			...
			GetPlayer( )->ChangeState( GetPlayer( )->GetPlayerState( PlayerRauder::STATE::ATTACK ) );
