...
//--------------------------------------------------------------------------------------
#ifndef _ENEMY_RAUDER_H_
#define _ENEMY_RAUDER_H_
...
//  プレイヤークラスの定義
//--------------------------------------------------------------------------------------
class EnemyRauder : public Scene
{
public:
	enum class STATE														//  状態
	{
	...
	static EnemyRauder*		Create( D3DXVECTOR3 position ,					//  生成
									D3DXVECTOR3 rot ,
									D3DXVECTOR3 scale ,
									float fSpeed ,
									MAGIC magic );
	...
	void				ChangeState( EnemyRauderState* enemyState );		//  状態の変化
	void				ChangeState( EnemyRauder::STATE state );			//  状態の変化
	...
	EnemyRauderState*	GetEnemyState( STATE state );						//  状態の取得
	EnemyRauderState*	GetCurrentEnemyState( void );						//  現在の状態の取得
...
private:
	...
	EnemyUI*			m_pEnemyRauderUI;									//  プレイヤーUIクラスのポインタ
	Magic*				m_pMagic;											//  魔法クラスのポインタ
	FBXLoader*			m_fbx;												//  FBXデータのポインタ

	EnemyRauderState*	m_enemyState;										//  プレイヤー状態クラスのポインタ

	EnemyRauderState*	m_allState[ ( int )STATE::MAX ];					//  状態全ての保存用

	::Effekseer::Handle m_handle[ EffekseerManager::TYPE_MAX ];				//  エフェクシアハンドル

	int					m_characterAIIndex;									//  キャラクターAI識別番号
