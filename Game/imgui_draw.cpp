...
void ImDrawList::AddPolyline(const ImVec2* points, const int points_count, ImU32 col, bool closed, float thickness, bool anti_aliased)
{
    ...
    if (anti_aliased)
    {
        ...
        if (!thick_line)
        {
            ...
            for (int i = 0; i < points_count; i++)
            {
                ...
                _VtxWritePtr[1].position = temp_points[i*2+0]; _VtxWritePtr[1].uv = uv; _VtxWritePtr[1].col = col_trans;
                _VtxWritePtr[2].position = temp_points[i*2+1]; _VtxWritePtr[2].uv = uv; _VtxWritePtr[2].col = col_trans;
                _VtxWritePtr += 3;
            }
        }
        else
        {
            ...
            for (int i = 0; i < points_count; i++)
            {
                _VtxWritePtr[0].position = temp_points[i*4+0]; _VtxWritePtr[0].uv = uv; _VtxWritePtr[0].col = col_trans;
                _VtxWritePtr[1].position = temp_points[i*4+1]; _VtxWritePtr[1].uv = uv; _VtxWritePtr[1].col = col;
                _VtxWritePtr[2].position = temp_points[i*4+2]; _VtxWritePtr[2].uv = uv; _VtxWritePtr[2].col = col;
                _VtxWritePtr[3].position = temp_points[i*4+3]; _VtxWritePtr[3].uv = uv; _VtxWritePtr[3].col = col_trans;
                _VtxWritePtr += 4;
            }
        }
    ...
    else
    {
        ...
        for (int i1 = 0; i1 < count; i1++)
        {
            ...
            _VtxWritePtr[0].position.x = p1.x + dy; _VtxWritePtr[0].position.y = p1.y - dx; _VtxWritePtr[0].uv = uv; _VtxWritePtr[0].col = col;
            _VtxWritePtr[1].position.x = p2.x + dy; _VtxWritePtr[1].position.y = p2.y - dx; _VtxWritePtr[1].uv = uv; _VtxWritePtr[1].col = col;
            _VtxWritePtr[2].position.x = p2.x - dy; _VtxWritePtr[2].position.y = p2.y + dx; _VtxWritePtr[2].uv = uv; _VtxWritePtr[2].col = col;
            _VtxWritePtr[3].position.x = p1.x - dy; _VtxWritePtr[3].position.y = p1.y + dx; _VtxWritePtr[3].uv = uv; _VtxWritePtr[3].col = col;
            _VtxWritePtr += 4;
...
void ImDrawList::AddConvexPolyFilled(const ImVec2* points, const int points_count, ImU32 col, bool anti_aliased)
{
    ...
    if (anti_aliased)
    {
        ...
        for (int i0 = points_count-1, i1 = 0; i1 < points_count; i0 = i1++)
        {
            ...
            // Add vertices
            _VtxWritePtr[0].position = (points[i1] - dm); _VtxWritePtr[0].uv = uv; _VtxWritePtr[0].col = col;        // Inner
            _VtxWritePtr[1].position = (points[i1] + dm); _VtxWritePtr[1].uv = uv; _VtxWritePtr[1].col = col_trans;  // Outer
            _VtxWritePtr += 2;
...
void ImFontAtlasBuildRenderDefaultTexData(ImFontAtlas* atlas)
{
    ...
    for (int type = 0; type < ImGuiMouseCursor_Count_; type++)
    {
        ...
        ImVec2 position = cursor_datas[type][0] + ImVec2((float)r.X, (float)r.Y);
        const ImVec2 size = cursor_datas[type][1];
        cursor_data.Type = type;
        cursor_data.Size = size;
        cursor_data.HotOffset = cursor_datas[type][2];
        cursor_data.TexUvMin[0] = (position) * tex_uv_scale;
        cursor_data.TexUvMax[0] = (position + size) * tex_uv_scale;
        position.x += FONT_ATLAS_DEFAULT_TEX_DATA_W_HALF + 1;
        cursor_data.TexUvMin[1] = (position) * tex_uv_scale;
        cursor_data.TexUvMax[1] = (position + size) * tex_uv_scale;
    }
}
...
void ImFont::RenderChar(ImDrawList* draw_list, float size, ImVec2 position, ImU32 col, unsigned short c) const
{
    ...
    if (const Glyph* glyph = FindGlyph(c))
    {
        ...
        position.x = (float)(int)position.x + DisplayOffset.x;
        position.y = (float)(int)position.y + DisplayOffset.y;
        draw_list->PrimReserve(6, 4);
        draw_list->PrimRectUV(ImVec2(position.x + glyph->X0 * scale, position.y + glyph->Y0 * scale), ImVec2(position.x + glyph->X1 * scale, position.y + glyph->Y1 * scale), ImVec2(glyph->U0, glyph->V0), ImVec2(glyph->U1, glyph->V1), col);
    }
}

void ImFont::RenderText(ImDrawList* draw_list, float size, ImVec2 position, ImU32 col, const ImVec4& clip_rect, const char* text_begin, const char* text_end, float wrap_width, bool cpu_fine_clip) const
{
    if (!text_end)
    ...
    while (s < text_end)
    {
        if (word_wrap_enabled)
        {
            ...
            if (!word_wrap_eol)
            {
                word_wrap_eol = CalcWordWrapPositionA(scale, s, text_end, wrap_width - (x - position.x));
                if (word_wrap_eol == s) // Wrap_width is too small to fit anything. Force displaying 1 character to minimize the height discontinuity.
                    word_wrap_eol++;    // +1 may not be a character start point in UTF-8 but it's ok because we use s >= word_wrap_eol below
            }
            ...
            if (c != ' ' && c != '\t')
            {
                ...
                if (x1 <= clip_rect.z && x2 >= clip_rect.x)
                {
                    ...
                    // We are NOT calling PrimRectUV() here because non-inlined causes too much overhead in a debug builds. Inlined here:
                    {
                        ...
                        vtx_write[0].position.x = x1; vtx_write[0].position.y = y1; vtx_write[0].col = col; vtx_write[0].uv.x = u1; vtx_write[0].uv.y = v1;
                        vtx_write[1].position.x = x2; vtx_write[1].position.y = y1; vtx_write[1].col = col; vtx_write[1].uv.x = u2; vtx_write[1].uv.y = v1;
                        vtx_write[2].position.x = x2; vtx_write[2].position.y = y2; vtx_write[2].col = col; vtx_write[2].uv.x = u2; vtx_write[2].uv.y = v2;
                        vtx_write[3].position.x = x1; vtx_write[3].position.y = y2; vtx_write[3].col = col; vtx_write[3].uv.x = u1; vtx_write[3].uv.y = v2;
                        vtx_write += 4;
                        vtx_current_idx += 4;
                        idx_write += 6;
                    }
