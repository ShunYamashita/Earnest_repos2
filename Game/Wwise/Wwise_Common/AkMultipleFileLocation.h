...
// "Going Further > Overriding Managers > Streaming / Stream SceneManager > Low-Level I/O"
...
public:
	...
	AKRESULT GetFullFilePath(
		const AkOSChar *	in_pszFileName,		// File name.
		AkFileSystemFlags * in_pFlags,			// Special flags. Can be nullptr.
		AkOpenMode			in_eOpenMode,		// File open mode (read, write, ...).
		AkOSChar *			out_pszFullFilePath, // Full file path.
		FilePath*			in_pBasePath = nullptr	// Base path to use.  If null, the first suitable location will be given.		
		);  
