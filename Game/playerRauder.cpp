...
//  プレイヤークラスのコンストラクタ
//--------------------------------------------------------------------------------------
PlayerRauder::PlayerRauder( ) : Scene( 3 )
{
...
//  プレイヤーの初期化処理
//--------------------------------------------------------------------------------------
HRESULT PlayerRauder::Init( void )
{
	m_drawDepth = true;
	...
	if( m_nPlayerNo == 0 )
	{
		...
		{
			//  プレイヤーUIの生成
			m_pPlayerRauderUI = PlayerUI::Create( PlayerUI::TYPE_PLAYER1 ,
	...
	else if( m_nPlayerNo == 1 )
	{
		...
		{
			//  プレイヤーUIの生成
			m_pPlayerRauderUI = PlayerUI::Create( PlayerUI::TYPE_PLAYER2 ,
	...
	//  プレイヤーステートの生成
	m_allState[ static_cast< int >( PlayerRauder::STATE::IDLE ) ] = new PlayerRauderIdle( this );
	m_allState[ static_cast< int >( PlayerRauder::STATE::MOVE ) ] = new PlayerRauderMove( this );
	m_allState[ static_cast< int >( PlayerRauder::STATE::JUMP ) ] = new PlayerRauderJump( this );
	m_allState[ static_cast< int >( PlayerRauder::STATE::ATTACK ) ] = new PlayerRauderAttack( this );

	//  ニュートラル状態で初期化
	m_playerState = m_allState[ static_cast< int >( PlayerRauder::STATE::IDLE ) ];
...
void PlayerRauder::Uninit( void )
{
	...
	//  プレイヤーステートの数分のループ
	for( int countState = 0; countState < static_cast< int >( PlayerRauder::STATE::MAX ) - 1; ++countState )
	{
...
//  プレイヤーの描画処理
//--------------------------------------------------------------------------------------
void PlayerRauder::Draw( void )
{
	Camera* camera = SceneManager::GetCamera( );

	D3DXVECTOR3 cameraVectorDirect;
	D3DXVECTOR3 cameraToPlayerRauder;

	if( camera != nullptr )
	{
		D3DXVECTOR3 cameraPosition = camera->GetCameraPosEye( );
		D3DXVECTOR3 playerRauderPosition = m_position;
		cameraPosition.y = 0.0f;
		playerRauderPosition.y = 0.0f;

		cameraToPlayerRauder = playerRauderPosition - cameraPosition;
		D3DXVec3Normalize( &cameraToPlayerRauder , &cameraToPlayerRauder );
	...
	if( Utility::IntoView( cameraVectorDirect , cameraToPlayerRauder , D3DX_PI * 0.5f ) )
	{
		if( m_fbx != nullptr )
		{
...
//  プレイヤーの描画処理
//--------------------------------------------------------------------------------------
void PlayerRauder::DrawDepth( void )
{
	if( m_fbx != nullptr )
	{
		m_fbx->SetDrawInfo( m_position , m_posAt );
...
//  地面との当たり判定をする関数
//--------------------------------------------------------------------------------------
bool PlayerRauder::JudgeHitGround( void )
{
	//  シーンクラスのポインタ
...
//  敵との当たり判定をする関数
//--------------------------------------------------------------------------------------
bool PlayerRauder::JudgeHitEnemy( void )
{
	//  シーンクラスのポインタ
	Scene* pScene = nullptr;	
		...
		for( int nCntPriority = 0; nCntPriority < MAX_NUM_PRIORITY; nCntPriority++ )
		{
			...
			while( pScene != nullptr )
			{
				...
				if( Game::GetModeVS( ) == Game::MODE_VS_PLAYER )
				{
					...
					{
						//  ダウンキャスト
						PlayerRauder* pPlayerRauder = ( PlayerRauder* )pScene;

						//  自分以外のプレイヤーでかつ、当たり判定をする場合
						if( m_nPlayerNo != pPlayerRauder->GetPlayerNo( ) && pPlayerRauder->GetJudgeHit( ) )
						{
							float fDist = 0.0f;

							if( Utility::HitSphere( m_hitSphere , pPlayerRauder->GetHitSphere( ) , &fDist ) )
							{
								//  速度をなくす
								m_fSpeed = 0.0f;

								//  反射するベクトルを求める
								D3DXVECTOR3 reflectVec = m_hitSphere.position - pPlayerRauder->GetHitSphere( ).position;
								reflectVec.y = 0.0f;
...
//  壁との当たり判定をする関数
//--------------------------------------------------------------------------------------
bool PlayerRauder::JudgeHitWall( void )
{
	bool hit = false;
...
//  インスタンス生成をする関数
//--------------------------------------------------------------------------------------
PlayerRauder* PlayerRauder::Create( D3DXVECTOR3 position , D3DXVECTOR3 rot , D3DXVECTOR3 scale , float fSpeed , MAGIC magic , int nPlayerRauderNo )
{
	PlayerRauder* pPlayerRauder;

	//  インスタンス生成
	pPlayerRauder = new PlayerRauder;

	//  座標の代入
	pPlayerRauder->m_position = position;

	//  最初の座標の代入
	pPlayerRauder->m_firstPos = position;

	//  注視点座標の代入
	pPlayerRauder->m_posAt = position;

	//  回転角の代入
	pPlayerRauder->m_rot = rot;

	//  大きさ倍率の代入
	pPlayerRauder->m_scale = scale;

	//  速度の代入
	pPlayerRauder->m_fBaseSpeed = fSpeed;

	//  所持魔法の代入
	pPlayerRauder->m_magic = magic;

	//  プレイヤー番号の代入
	pPlayerRauder->m_nPlayerNo = nPlayerRauderNo;

	//  初期化
	pPlayerRauder->Init( );

	return pPlayerRauder;
}

//--------------------------------------------------------------------------------------
//  プレイヤーのダメージ処理
//--------------------------------------------------------------------------------------
void PlayerRauder::Damage( D3DXVECTOR3 blowVecDirect , float fBlowPower , int nDamage , bool bBlow )
{
	if( m_nInvisibleTime != 0 )
	{
		return;
	...
	if( m_pPlayerRauderUI != nullptr )
	{
		...
		if( m_nPlayerNo == 0 )
		{
			m_pPlayerRauderUI->SetSwaySide( fSwaySide );
		}
		else if( m_nPlayerNo == 1 )
		{
			m_pPlayerRauderUI->SetSwaySide( -fSwaySide );
		...
		m_pPlayerRauderUI->SetSwayVertical( fSwayVertical );
	}
...
//  プレイヤーのブレイブダメージ処理
//--------------------------------------------------------------------------------------
int PlayerRauder::BraveDamage( int nDamage )
{
	if( m_nInvisibleTime != 0 )
	{
		return 0;
...
//--------------------------------------------------------------------------------------
void PlayerRauder::StopAllEffekseer( void )
{
...
//  ロックオン状態の変更
//--------------------------------------------------------------------------------------
void PlayerRauder::ChangeLockon( void )
{
	m_bLockOn = !m_bLockOn;
}
...
//  現在のジャンプ力の設定
//--------------------------------------------------------------------------------------
void PlayerRauder::SetCurrentJumpPower( float jumpPower )
{
...
//  方向ベクトルの設定
//--------------------------------------------------------------------------------------
void PlayerRauder::SetVecDirect( D3DXVECTOR3 vecDirect )
{
...
//  注視点の設定
//--------------------------------------------------------------------------------------
void PlayerRauder::SetPositionAt( D3DXVECTOR3 positionAt )
{
	m_posAt = positionAt;
}
...
//  速度の設定
//--------------------------------------------------------------------------------------
void PlayerRauder::SetSpeed( float speed )
{
	m_fSpeed = speed;
}
...
//  移動力の設定
//--------------------------------------------------------------------------------------
void PlayerRauder::SetMovePower( float movePower )
{
...
//  吹っ飛び力の設定
//--------------------------------------------------------------------------------------
void PlayerRauder::SetBlowPower( float blowPower )
{
...
//  無敵時間の設定
//--------------------------------------------------------------------------------------
void PlayerRauder::SetInvisibleTime( int invisibleTime )
{
...
//  追尾方向の設定
//--------------------------------------------------------------------------------------
void PlayerRauder::SetHomingDirect( bool homingDirect )
{
...
//  指定した種類のエフェクシア識別子の設定
//--------------------------------------------------------------------------------------
void PlayerRauder::SetEffekseerHandle( EffekseerManager::TYPE type , ::Effekseer::Handle handle )
{
...
//  敵と当たり判定するかのフラグ設定
//--------------------------------------------------------------------------------------
void PlayerRauder::SetJudgeHit( bool judgeHit )
{
...
//  防御フラグの設定
//--------------------------------------------------------------------------------------
void PlayerRauder::SetGuardFlag( bool guardFlag )
{
	m_bGuard = guardFlag;
}
...
//  魔法クラスのポインタの設定
//--------------------------------------------------------------------------------------
void PlayerRauder::SetMagic( Magic* magic )
{
	m_pMagic = magic;
}
...
//  モーションの設定
//--------------------------------------------------------------------------------------
void PlayerRauder::SetMotion( FBXLoader::MOTION motion )
{
	if( m_fbx != nullptr )
	{
...
//  体力の増加
//--------------------------------------------------------------------------------------
void PlayerRauder::AddLife( int addLife )
{
	m_nLife += addLife;
}
...
//  魔法の削除
//--------------------------------------------------------------------------------------
void PlayerRauder::DeleteMagic( void )
{
	//  魔法を所持している場合
	if( m_pMagic != nullptr )
	{
...
//  方向ベクトルの取得
//--------------------------------------------------------------------------------------
D3DXVECTOR3	PlayerRauder::GetVecDirect( void )
{
	return m_vecDirect;
}
...
//  注視点の取得
//--------------------------------------------------------------------------------------
D3DXVECTOR3	PlayerRauder::GetPositionAt( void )
{
	return m_posAt;
}
...
//  吹っ飛び方向ベクトルの取得
//--------------------------------------------------------------------------------------
D3DXVECTOR3	PlayerRauder::GetBlowVecDirect( void )
{
...
//  状態の変化
//--------------------------------------------------------------------------------------
void PlayerRauder::ChangeState( PlayerRauderState* playerState )
{
	//  同じの場合リターン
...
//  プレイヤー番号の取得
//--------------------------------------------------------------------------------------
int	PlayerRauder::GetPlayerNo( void )
{
	return m_nPlayerNo;
}
...
//  無敵時間の取得
//--------------------------------------------------------------------------------------
int	PlayerRauder::GetInvisibleTime( void )
{
...
//  魔法クラスのポインタの取得
//--------------------------------------------------------------------------------------
Magic* PlayerRauder::GetMagic( void )
{
	return m_pMagic;
}
...
//  魔法の種類取得
//--------------------------------------------------------------------------------------
PlayerRauder::MAGIC PlayerRauder::GetMagicType( void )
{
	return m_magic;
}
...
//  速度の取得
//--------------------------------------------------------------------------------------
float PlayerRauder::GetSpeed( void )
{
	return m_fSpeed;
}
...
//  基本速度の取得
//--------------------------------------------------------------------------------------
float PlayerRauder::GetBaseSpeed( void )
{
	return m_fBaseSpeed;
}
...
//  ジャンプ力の取得
//--------------------------------------------------------------------------------------
float PlayerRauder::GetJumpPower( void )
{
	return m_fJumpPower;
}
...
//  現在のジャンプ力の取得
//--------------------------------------------------------------------------------------
float PlayerRauder::GetCurrentJumpPower( void )
{
...
//  目標までの距離の取得
//--------------------------------------------------------------------------------------
float PlayerRauder::GetTargetDistance( void )
{
	//  シーンクラスのポインタ
	Scene* pScene = nullptr;											
		...
		while( pScene != nullptr )
		{
			...
			else if( Game::GetModeVS( ) == Game::MODE_VS_PLAYER )
			{
				if( objType == Scene::OBJTYPE_PLAYER )
				{
					...
					//  自分以外のプレイヤー番号の場合
					if( m_nPlayerNo != pPlayerRauder->GetPlayerNo( ) )
					{
						//  敵の座標の代入
						D3DXVECTOR3 posEnemy = pPlayerRauder->GetPos( );
...
//  移動力の取得
//--------------------------------------------------------------------------------------
float PlayerRauder::GetMovePower( void )
{
	return m_fMovePower;
}
...
//  吹っ飛び力の取得
//--------------------------------------------------------------------------------------
float PlayerRauder::GetBlowPower( void )
{
	return m_fBlowPower;
}
...
//--------------------------------------------------------------------------------------
bool PlayerRauder::GetJudgeHit( void )
{
	return m_bJudgeHit;
}
...
//--------------------------------------------------------------------------------------
bool PlayerRauder::GetGuard( void )
{
	return m_bGuard;
}
...
//  ロックオン情報の取得
//--------------------------------------------------------------------------------------
bool PlayerRauder::GetLockon( void )
{
	return m_bLockOn;
}
...
//--------------------------------------------------------------------------------------
bool PlayerRauder::GetHomingDirect( void )
{
...
//  モーションが終了したかの取得
//--------------------------------------------------------------------------------------
bool PlayerRauder::GetMotionFinish( void )
{
	if( m_fbx != nullptr )
	{
...
//  状態の取得
//--------------------------------------------------------------------------------------
PlayerRauderState* PlayerRauder::GetPlayerState( PlayerRauder::STATE state )
{
...
//  エフェクシア識別子の取得
//--------------------------------------------------------------------------------------
::Effekseer::Handle PlayerRauder::GetEffekseerHandle( EffekseerManager::TYPE type )
{
...
//  ジャンプでの移動
//--------------------------------------------------------------------------------------
void PlayerRauder::MoveHeight( float addMove )
{
...
//  ジャンプ力を下げる
//--------------------------------------------------------------------------------------
void PlayerRauder::DownCurrentJumpPower( float downPower )
{
