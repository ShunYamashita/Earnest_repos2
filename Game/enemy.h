...
public:
	...
	void				ChangeState( EnemyState* enemyState );				//  状態の変化
	...
	EnemyState*			GetEnemyState( STATE state );						//  状態の取得
	EnemyState*			GetCurrentEnemyState( void );						//  現在の状態の取得
...
private:
	...
	EnemyState*			m_enemyState;										//  エネミー状態クラスのポインタ

	EnemyState*			m_allState[ ( int )STATE::MAX ];					//  状態全ての保存用
